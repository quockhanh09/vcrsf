<template>
  <section class="min-h-[50vh] flex flex-col">
    <div class="border-b border-borderGray">
      <div class="container py-8 flex justify-between items-center">
        ...
        <div class="relative px-2 text-2xl font-bold cursor-pointer">
          ...
          <img
            class="absolute top-1 -right-1 -z-10 cursor-pointer"
            v-lazy="downArrowIc"
            alt="down arrow ic"
          />
        </div>
    ...
    <transition name="slide-translate" mode="out-in">
      <div v-if="!loadingEvent">
        <div v-if="events.length">
          <div
            class="py-8 border border-borderGray"
            :class="e.status === 'Đã diễn ra' ? 'text-gray' : 'text-black'"
            v-for="e in events"
            :key="e.pk"
          >
              ...
              <div class="w-1/3 flex justify-start items-start gap-[10px]">
                <div class="uppercase text-center">
                  <h5 class="text-4xl font-bold">
                    {{ formatDate(e.fields.dates) }}
                  </h5>
                  <p class="text-lg font-bold">
                    {{ formatDayOfWeek(e.fields.dates) }}
                  </p>
                </div>
                <div class="mt-4 uppercase">
                  <h5>Tháng {{ formatMonth(e.fields.dates) }}</h5>
                </div>
              </div>
              <div class="w-2/3">
                <p class="my-4 text-2xl font-bold">{{ e.fields.name }}</p>
                ...
                <button
                  ...
                  :class="
                    e.status === 'Đã diễn ra'
                      ...
                      : 'view-detail'
                  "
                  @click="() => linkToComponents(e.fields.status, e.pk)"
                >
                  {{
