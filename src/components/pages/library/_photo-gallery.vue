<template>
  <section class="mb-16">
    ...
    >
      <div class="absolute w-full h-full -z-20">
        <img
          class="w-full h-full"
          v-lazy="thuvienanhImg"
          alt="Thư viện ảnh img"
        />
      </div>
      <div class="container">
        <h3
  ...
  <transition name="slide-translate" mode="out-in">
    <section v-if="!loadingPhotos" class="container photo-gallery">
      <div class="flex items-end" v-for="photo in photos" :key="photo.id">
        ...
        <div class="w-full p-[20px] text-white">
          <div>
            <p class="text-2xl font-bold">{{ formatDay(photo?.created) }}</p>
          </div>
          <div>
          ...
          <div class="flex justify-between items-center">
            ...
            <div
              class="w-1/3 m-auto flex justify-center items-center gap-[10px]"
            >
